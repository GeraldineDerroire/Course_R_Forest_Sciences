...
Let's imagine that we have several logs for which we know the volume, and we want to calculate the mean volume of a log, we can to:
...
[<span style="font-size: 20px">🤔 What is a categorical variable?</span>]{.fragment}
[<span style="font-size: 20px">It's a type of variable representing data divided in distinct group or categories.</span>]{.fragment}
...
::: notes
explain what is in the print:
first line : the values
second line: the levels
...
We are quickly present the other ones now, without 
explaining much how to work with them.
We will see that later.

The main ones that we are going to use during the class are:
...
::: notes
Let's not spend some time on this, you won't need ite for the moment,
it's enought to know that this exist.
:::
...
::::: columns
::: {.column width="50%"}
<span style="font-size: 25px">The objects contained in a list can also be named when we create the list:</span>
...
::: {.column width="50%"}
::: fragment
<span style="font-size: 25px">We can then access an object by its name, using *$*:</span>
```{r}
L1$Volume
```
:::
