...
::: notes
discuss a bit with people who already use R

=\> what do they find particularly difficult?
easy?

we are not going to work directly in R but in RStudio => close R 
:::
...
RStudio is an *Integrated Development Environment* (IDE): a software developed by Posit that provides many facilities to help programming with R.
...
## Objects are not necessarily numbers
...
We can use the function *typeof* to check what is the type of an object:
...
Here are the main types you will use:

```{r, echo=FALSE}
mod <- data.frame(Type = c("double", "integer", "character", "logical"),
                  What = c("real number", "integer number", "character string", "bolean (TRUE/FALSE)"),
                  Example = c("2.3", "2", " \"hello\" ", "TRUE"))
knitr::kable(mod)
```
...
Numeric values can be stored as different types:
...
What happens if we try to put a decimal number as an integer ?
...
::: fragment
To save a script, go to *File \> Save* or *File \> Save As...*. 

You can also use the shortcut *Ctrl + S* or click on 💾.
:::

::: fragment
To open an existing script, go *File \> Open file...*.

💡 You can also use the short cut *Ctrl + O*📂.
:::
...
::: fragment
[For next session, create a project for the course.]{style="color:indianred;"}

Create a raw data folder and put in it all the datasets downloaded during the installation phase.
:::

::: notes
This is important, we will need the data for the future class.
:::
...
A vector can contain several values of any type...
...
[...but these values need to be **of the same type**]{.fragment}
...
[R converted all the values into character (the more flexible type).]{.fragment}
...
[When printing a factor, the first line shows the values, the second line shows the levels.]{.fragment}
...
legacy S-language classification that groups related storage types together

class() returns the high-level object-oriented property that dictates function behavior
...
A matrix is a **two-dimensional** data structure (rows and columns) where all the element are of the **same type**.
...
rowSums :  sum all the value of a row

rowsum: sum rows belonging to a same group
:::
...
An array is a **multi-dimensional** data structure where all the element are of the **same type**.
...
* A package gathers several functions for a same thematic
...
* Damiani, A., Milz, B., Lente, C., Falbel, D., Correa, F., Trecenti, J., Luduvice, N., Lacerda, T., & Amorim, W. *Ciência de Dados em R. Curso-R.* (Livro em portuguese, até capitulo 4) [Here](https://curso-r.github.io/livro-material/){preview-link="false"} 
