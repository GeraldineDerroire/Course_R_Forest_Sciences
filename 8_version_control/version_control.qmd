...
-   experiment safely as you can recover from mistakes
...
[You can use git without GitHub]{style="font-size: 30px"}
...
-   a [GitHub]{style="color:indianred;"} account connected to Git using SSH
:::
...
**Step1: [Stage your changes]{style="color:indianred;"} the changes**
...
[But we will do most of them from **RStudio**, which provides an interface for git.]{style="font-size: 30px"}
...
[Create the GitHub repository first, then clone it.]{style="font-size: 29px"}
...
[💡 You can still make a repo public if you have information (passwords) or data that you don't want to share, by including them in the *.gitignore*, [but only if those files have never been committed]{style="color:red;"}.]{style="font-size: 30px"}
...
[This will synchronize your local repository with the remote one, downloading any new commits made by your collaborators, or by yourself directly on GitHub.]{style="font-size: 30px"}
...
Let's make an example to practice together by creating a problem:
...
Let's generate a conflict to practice together:
...
* [*On the local repo*]{style="font-size: 30px"}
    ...
    * [*Don't pull yet*]{style="font-size: 30px"}
...
* [Communicate with collaborators: *Know who is working on what — avoid editing the same files at the same time*]{style="font-size: 30px"}
...
# GitHub as a research portfolio
...
## GitHub as a research portfolio {style="font-size: 30px"}
...
* Share code accompanying publications
