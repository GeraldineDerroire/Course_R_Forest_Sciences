...
## The package [ggplot2]{style="color:indianred;"}{.smaller}

::::: columns
::: {.column width="80%"}

* [ggplot2]{style="color:indianred;"} is a package of the *tidyverse* allowing to make plots using a coherant syntax.

* [ggplot2]{style="color:indianred;"} works with data frames (including tibbles).
...
[The function [ggplotly]{style="color:indianred;"} of the package [plotly]{style="color:indianred;"} can transform many ggplot2 figures into interactive graphics:]{style="font-size: 30px"}
...
R provides powerful tools for spatial data analysis and mapping, ... but GIS is beyond the scope of this module.

For a full introduction to GIS with R see [Geocomputation with R](https://r.geocompx.org/) and [Spatial Data Science with applications in R](https://r-spatial.org/book/).


## [ggspatial]{style="color:indianred;"}: a  ggplot2 extension for maps {.smaller}



::::: columns
::: {.column width="40%"}
The package [ggspatial]{style="color:indianred;"} works with spatial vector data such as shapefiles.

See the [vignette of ggspatial](https://paleolimbot.github.io/ggspatial/index.html) for more.
:::

::: {.column width="60%"}
```{r}
library(ggspatial)
load_longlake_data() # load some spatial objects

ggplot() +
  annotation_spatial(longlake_waterdf) +
  layer_spatial(longlake_depthdf, aes(colour = DEPTH_M)) +
