...
# Improving the visual appearance of graphics

::: notes
so far we have seen how to quickly make graphs to explore and visualise our data
But to communicate with others, we want to make graphs explicit and pretty
:::

## {.smaller}

[Let's go back to a plot we already made, save it, and start to improve the visual appearance:]{style="font-size: 28px"}
...
graph1 <-  ggplot(dt_sub, aes(x = D, y = H, color = genus)) +
  geom_point() +
...
::: notes
note how we add things on the graph by adding + to an existing graphic
:::
...
Let's not change graph1 (see no <-)
:::

## Replace a color scale
::::: columns
::: {.column width="23%"}
...
[Use [RColorBrewer::display.brewer.all()]{style="color:indianred;"} for all available palettes]{style="font-size: 15px"}
:::
...
theme(legend.position (see 11.4.3))
