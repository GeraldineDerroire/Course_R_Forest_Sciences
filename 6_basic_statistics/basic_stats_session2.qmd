...
preds <- ggpredict(mod_dh_2, terms = "D")  
# default gives the confidence interval
...
They now have a mean of 0 and a standard deviation = 1:]{style="font-size: 30px"}]{.fragment}
...
<!-- TO DO see on the piece of paper -->

## Model validation {.smaller}

<!-- TO DO with my new model with interaction-->
...
library(lindia)
gg_resX(mod_hills, ncol = 2)

<!-- TO DO with ggdiagnose cf Marchanc -->

## Collinearity

<!-- TO DO with my new model with interaction-->

hyp supplementaire à tester (le faire sur hills)
...
ggcoef_model of ggstats (see in Lamarange)

and also a table with the following, which work as well with a simple model


```{r}
tbl_regression(mod_dh_2, intercept = TRUE)
```

## Presenting the results

ggstats ggcoef_model (Lamarange)

## Presenting the results

gg

marginal prediction with ggpredict: https://larmarange.github.io/guide-R/analyses/estimations-marginales.html#variantes
