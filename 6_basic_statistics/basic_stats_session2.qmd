...
TELL WHAT ARE THE CONFIDENCE INTERVAL + COMMENT THE CODE

```{r}
new <- tibble(D = seq(from = 10, to = 160, by= 10))
new_conf <- predict(mod_dh_1, newdata = new,
                    interval = "confidence")
new_conf <- bind_cols(new, new_conf)
slice_head(new_conf)
```
...
gpred <- ggplot(new_conf, aes(x= D)) +
  ...
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3, color = "darkgrey") +
  geom_point(data = dt_nou, aes(x = D, y = H), alpha = 0.3) +
  theme_bw()
gpred
```
