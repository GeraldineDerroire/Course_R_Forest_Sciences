---
title: "Statistics with R - Distributions and hypothesis testing"
...
I don't know your level of stats, so please ask when unclear !!!
:::
...
Now we are going to see two very common distributions
...
[$a$ and $b$ are the two parameters of an uniform distribution. $[a, b]$ is the range of possible values (so *a* is the minimum value and *b* the maximum value)]{style="font-size: 30px"}
...
```{r}
runif(n = 10, # number of value to draw in the distribution
...
[[💡 When two variables are measured in different scales, it can be usefull to standardise them by centring and reducing them, to be able to compare them.]{style="font-size: 30px"}]{.fragment}
...
::: fragment
```{r}
# take a subset of dt_HD containing 
# the 10 most abundant genus
dt_sub10 <- dt_HD %>% 
...
[We can look at the percentages per column (*here* the relative proportion of species in a given plot), using the function [cprop]{style="color:indianred;"} of the package [questionr]{style="color:indianred;"}:]{style="font-size: 30px"}
...
::: fragment
[⚠️ Here we look at the frequency per column because we are interested in the effect of the plot, and the plot is in column.]{style="font-size: 30px"} 

[The function [lprop]{style="color:indianred;"} also exists to look at frequency per row.]{style="font-size: 30px"}
:::
...
[We can then perform a $\chi^2$ test, to test the independence of the two categorical variables.]{style="font-size: 30px"}
...
::: notes
for a χ² test of independence on a contingency table is:

df = (nb row -1) * (nb col - 1) = (10-1) * (2-1) = 9
...
::: notes
classic guideline => Cochran's rule is: no expected cell below 1, and no more than ~20% of cells below 5

Here for instance, considering the total number of Dicorynia, and the number of trees in each plot, if the abundance didn't depend on the plot, we would have 8.66 Dicorynia in plot 1 and 14.33 in plot 2
...
[We can look at the residuals (here the *standardised residuals*) using the function [chisq.residuals]{style="color:indianred;"} of the package [questionr]{style="color:indianred;"}:]{style="font-size: 30px"}
...
* [standardised residuals < -2 indicate **under-representation**]{style="font-size: 28px"}

* [standardised residuals > 2 indicate **over-representation**]{style="font-size: 28px"}

* [standardised residuals in [-2, 2] indicate **no significant difference** to independence]{style="font-size: 28px"}
...
::: notes
residuals are a measurment of how different are observed from the predicted predicted under the null hypothesis

std residuals = (obs_ij - exp_ij) / squareroot (exp_ij * (1 - pi.) * (1 - p.j))

ex: dicorynia in plot 1 => res = (23 - 8.66) / sqrt(8.66 * (1- (23/467)) * (1 - (208/467)))
...
::: {.column width="35%"}
[[The result is significant, so we can reject the null hypothesis that the means of both groups are equal and conclude that the two genus differ in height.]{style="font-size: 30px"}]{.fragment}
:::
:::::
...
[The ANOVA assumes **homoscedasticity**, meaning a constant variance of residuals across groups.]{style="font-size: 30px"}
...
Here, as the fitted value is the mean of the group (the genus), we have a bar per genus
:::
...
[**We can consider that the assumption of normality of the distribution of the residuals is respected.**]{style="font-size: 30px"}
:::
:::
:::::
...
[[The other coefficients show the difference of each group with the first group. For instance, the mean of *Sterculia* is:]{style="font-size: 30px"}]{.fragment}
...
[And we make the plot:]{style="font-size: 30px"}
...
graph_H <- ggplot() +
  ...
  theme_bw() # because it looks nicer !
```
...
last slide for today, with the graph we are starting to prepare for linear models => next session
:::
