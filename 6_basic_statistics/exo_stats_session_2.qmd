...
The main island (called *Britain*) is much bigger than the other ones. 
For the purpose of these exercices, remove this observation from the data.

```{r}
dt <- dt %>% filter(island != "Britain")
```
...
# interpretation
  # No very influential observations, the distance of Cook are small
```
...
GGally::ggpairs(dt, 
                columns = c("log_area", "elevation",
                            "latitude", "dist_britain"),
...
# interpretation
  # No very influential observations, but some obs concerning
```


```{r}
dt %>%
  mutate(resid = residuals(mod_2)) %>% # add the residuals
  pivot_longer(cols = c(log_area, latitude),
               names_to = "predictor",
...
# in the corrected exercice, the table might be above
```
...
```{r}
preds4area <- predict_response(mod_2, terms = "area")
preds4area
# str(preds4area)
```
...
```{r}
preds4lat <- predict_response(mod_2, terms = "latitude")
preds4lat
# str(preds4lat)
```
