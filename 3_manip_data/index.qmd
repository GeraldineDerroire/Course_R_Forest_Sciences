...
[We use the command ]{.fragment}[data]{style="color:indianred;"} to open the data:
...
[or to click on the table icon in the environment: ![](view.png){width="80%"}]{.fragment}
...
[We can also use the function ]{.fragment}[dim]{style="color:indianred;"} to get both dimention (1st the number of rows, second the number of columns).
...
[To see only the first or last values, we use the functions ]{.fragment}[head]{style="color:indianred;"} or [tail]{style="color:indianred;"}.
We can specify the number of observations to show in the second argument:
...
[The function ]{style="font-size: 23px"}[rep]{style="color:indianred;"} allows to repeat the same value (here "BCI") a certain number of times (here 50).
...
[We can transform the variable *site* to a factor using the function ]{.fragment}[as.factor]{style="color:indianred;"}
...
::: notes
descriptive statistics are summary statistics They help us to get a fell of our data Important to use them often
:::
...
[We can get the median using the function ]{.fragment}[median]{style="color:indianred;"}
...
[We can also get the **variance** and the **standard deviation** (which is the square root of the variance), using the functions ]{.fragment}[var]{style="color:indianred;"} and [sd]{style="color:indianred;"}
...
depending how late it is =\> explore them a bit or not
...
<!-- tell that we can also import from other format but bof-->
...
-   Metadata, sometimes referred to as data about data or a data dictionary, are a “centralized repository of information about data such as meaning, relationships to other data, origin, usage, and format” (IBM Dictionary of Computing, 10th edition, 1993). In Jenkins et al 2023
