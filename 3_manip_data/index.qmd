...
* or in the *Environment*, *From Text (base)...*
...
BCI.env <- read.csv(file = "data/BCI_env.csv", # the file name and its relative path
                    header = TRUE) # the dathave headers (column names)
```

[By default, the the separator of columns is the [,]{style="color:indianred;"} and the separator of decimal the [.]{style="color:indianred;"}]{.fragment}
...
We can save a data frame to a csv file using the function [write.csv]{style="color:indianred;"}:
