...
Let's consider the variable *environmental heterogeneity* (EnvHet) (see the help to know how it was calculated).
...
[Let's look at what is in the file [BCI_env.csv](https://geraldinederroire.github.io/Course_R_Forest_Sciences/3_manip_data/data/BCI_env.csv)]{.fragment}
...
[[NB: The function *read_csv* from the **tidyverse** handles encodings more efficienty.]{style="font-size: 25px"}]{.fragment}

::: notes
We'll see the tidyverse in the next session.
:::
...
```{r}
library(readxl)
BCI.env <- read_excel(path = "data/BCI_env.xlsx")
```

*NB: the function read_excel returns a tibble, we will see what this is in the next session.*

But using a universal data format (CSV) is preferred.

## Typing data in a spreadsheet

If you collect data in the field on a paper field sheet,
you can then use Excel, Google Sheets, LibreOffice Calc... 
to digitalise them.
...
## Using digital field solutions - [KoboCollect]{style="color:indianred;"}

::::: columns
::: {.column width="65%"}
* Free and easy to use 
* Forms created via a web interface using **KoboToolbox** and deployed on smartphones/tablets with the **KopoToolbox mobile app**
* Possible to collect data offline

[[Tutorial for KoboToolbox](Kohttps://support.kobotoolbox.org/welcome.html)]{style="font-size: 25px"}
:::
...
## Using digital field solutions - [Arena]{style="color:indianred;"}

::::: columns
::: {.column width="65%"}

* Free and open-source 
* Developped by the FAO for forest inventories
* Forms created via a web interface using **Arena** and deployed on smartphones/tablets with **Arena mobile**
* Possible to collect data offline
...
[[Tutorial for Arena](https://www.youtube.com/watch?v=EIzt4rpi36A&list=PL0Rrgop7D4QAWSJMtRQojzKuhF4vPS6Rs&index=12)]{style="font-size: 25px"}

## Using digital field solutions - [Qfield]{style="color:indianred;"}

::::: columns
::: {.column width="65%"}

* Free and open-source GIS app
* All entries are spatialised
* Forms created in a **QGis** project and deployed on smartphones/tablets with **Qfield**
* Possible to collect data offline
...
## Writing data from R to a csv

From R, We can save a data frame to a csv file using the function [write.csv]{style="color:indianred;"}:
...
::: notes
We can do this either because we have created the data in R
or because we import them in R and modified them.
:::
...
* in paper field sheets (in that case, also save the paper sheet on which data are first recorded)
...
* the type of each variable [*For example, if a numerical data is stored in a character vectors, there must be a typo somewhere.*]{.fragment}
...
* Fabrice Bénédet (CIRAD) for digital field solutions
...
## Ressources to go further {.smaller}

::: {.nonincremental}

**On data format**

* *Data formats* - Nicolas Casajus [Here](https://biodiversitydata.github.io/chapters/files-formats.html){preview-link="false"}
