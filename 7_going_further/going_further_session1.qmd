...
```
::: notes
test data don't necessaryly need to be created from scratch, they can be a subset of our real data
:::
...
*We would like to round the mean diameter, let's add an new operation and an argument giving the number of decimals.*
...
```{r}
mean_diam_func <- function(circ, na_act, digits) {
  # Calculate diameters in m
    diam <- circ / (pi * 100)
  # Calculate the mean value
    mean_diam <- mean(diam, na.rm = na_act)
...
```{r}
# first test
 mean_diam_func(circ = new_circ, digits = 1, na_act = TRUE)
```
...
```{r}
# second test
mean_diam_func(circ = new_circ, digits = 4, na_act = TRUE)
```
:::
...
```{r}
# third test
mean_diam_func(circ = new_circ, digits = 3, na_act = FALSE)
```
:::
...
[We can also include a [message] in a function:]{style="font-size: 30px"}
...
[Let's go back to our [mean_diam_func]{style="color:indianred;"} function:]{style="font-size: 30px"}

```{r, eval = FALSE}
mean_diam_func(new_circ)
```

```{r, echo = FALSE}
try(mean_diam_func(new_circ))
```

[and specify **default** values that are taken by the arguments if no values is passed :]{style="font-size: 30px"}

```{r}
mean_diam_func2 <- function(circ, 
...
mean_diam_func2(new_circ)
```
[[⚠️ Argument with a default value are *optional*, but the other ones are *compulsory*.]{style="font-size: 30px"}]{.fragment}
...
cf Juba: https://juba.github.io/tidyverse/15-dplyr-avance.html#appliquer-ses-propres-fonctions

pour l'exemple de mutate, repartir des données de l'IFN brésilien
