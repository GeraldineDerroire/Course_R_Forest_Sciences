...
::: notes
we are going to see in a few minutes what are tidy data and pipe
:::
...
we see that this attach several packages

talk about conflicts and masked functions => dyplr function used by default
so need to specify the package if we want to use the functions from the package stats
:::
...
::: notes
today: what is a tibble + packages readr, dplyr
next session: tidyr, a bit of stringr and forcats
latter: session on ggplot2
:::
...
Ex for one table per obs unit : trees and plots => 2 tables
:::
...
<!--
[It can be accessed using the shortcut [Ctrl + Shift + m]{style="color:indianred;"}]{.fragment}
-->

[[Since R 4.1, base R also includes the native pipe [|>]{style="color:indianred;"}. 
However, in this course we will use %>%, which remains widely used throughout the tidyverse.]{style="font-size: 30px"}]{.fragment}
:::
...
but were are going to import a modified version from a csv file available 
...
[⚠️ These datasets should be placed in the data folder of your RStudio project.]{style="font-size: 30px"}
...
Here we get a nice message when importing data. It tells us...
:::
...
::: notes
we see that the data are a tibble (tbl) AND a data.frame AND a tbl_df AND a spec_tbl_df
...
::: notes
Let's see another way of getting a tibble

! note read.csv and read_csv
:::

## Transform data frame into tibble {.smaller}
...
## Commercial tree species {.smaller}
...
[To know more, we can look at the help on the package LoggingLab (if you want to install it, see instructions [here](https://vincyanebadouard.github.io/LoggingLab/))]{.fragment}
...
# Manipulate and transform data with the package [dplyr]{style="color:indianred;"}
...
::: notes
we are not going to see all the functions, but the more common and usefull ones
:::
...
[From now on, we will adopt this way of writing.]{.fragment}
...
```{r}
traits %>% 
  slice_head(n=3) # n specify the number of rows
```

::: fragment
```{r}
traits %>%
  slice_tail(prop=0.1) # prop specify the proportion of rows
```
:::

::: notes
The number of rows can be specify or we can specify a proportion of rows (here 10%, we had 18 species => 1 row)
:::
...
```{r}
sp_com %>% 
  filter(Aggregative == TRUE)
```

::: notes
only 2 aggregative species 

talk about what is an aggregative species
:::

## filter

Or let's select the species that have a wood specific gravity (wsg) lower than 0.6:
...
traits %>% 
  filter(
    s_wsg < 0.6 & # first condition
      s_LMA < 90) # second condition
```
[There are no species satisfying these two conditions in our dataset.]{.fragment}

::: notes
look at the syntax together,especially the &
...
```{r}
traits %>% 
  filter(s_dbhmax > median(s_dbhmax))
```

::: notes
in the dataset we have 18 values, so when we get the one above the median we get 9
=> it makes sense
:::
...
```{r}
sp_com %>% 
  select(CommercialName, Genus, Species)
```

## select {.smaller}

We can also specify the columns we don't want to select using [select]{style="color:indianred;"}.
...
```{r}
traits %>% 
  rename(LMA = s_LMA) # new name = old name
```
...
```{r}
traits %>% 
  mutate(dbhmax_cm = s_dbhmax * 100) %>% # add the new column dbhmax_cm
  select(genus, species, s_dbhmax, dbhmax_cm) %>%  # select only these 4 columns 
  slice_head(n=5) # just the 5 first rows
```


::: notes
mutate is very usefull!!!!
...
```{r}
traits %>% 
  relocate(genus, species, s_dbhmax, s_hmax) %>% 
  slice_head(n=5) # just the 5 first rows
```
...
[The function [group_by]{style="color:indianred;"} allow grouping rows according to the values of a column.]{style="font-size: 30px"}
...
[[There is no apparent change in the tibble, except the mention of the grouping.]{.fragment}]{style="font-size: 30px"}
...
## group_by with slice {.smaller}

[When groups have been defined, we can apply further operations on these groups by using other functions.]{style="font-size: 25px"}
...
[[We see that the resulting tibble has as many rows as there are genus.]{.fragment}]{style="font-size: 25px"}
...
[We can also use [group_by]{style="color:indianred;"} with [mutate]{style="color:indianred;"} to do an operation by group.]{style="font-size: 25px"}
...
traits %>% 
  ...
  select(genus, species, s_wsg, mean_wsg) # show only these colums
```
:::
...
::: {.column width="30%"}
[[Here all rows are kept and species of a same genus have the same value for mean_wsg]{.fragment}]{style="font-size: 25px"}
:::
:::::


::: notes
all rows are kept as we didn't do a selection on rows
:::


## group_by with filter {.smaller}
...
::: notes
Here some of the genus have just one species, so their wsg is not stricly > to the mean
:::

## summarise

The function [summarise]{style="color:indianred;"} aggregates the rows by performing a summary operation on one or several columns.
...
[*Note that here only the summary columns and only one row are kept.*]{.fragment}

::: notes
and only one row of summary
:::
...
::: {.column width="30%"}
[[We see that the genus *Couepia* has two species, for instance.]{style="font-size: 25px"}]{.fragment}

:::
:::::

## Grouping according to several columns {.smaller}
...
[[We see that the genus *Caryocar* has two species for which the commercial name is chawari.]{style="font-size: 25px"}]{.fragment}
:::
:::::
...
## Using a pipe to chain the different operations  {.smaller}
...
::: notes
This is the end of the session 1/2
::: 
...
# Combining multiple tables with the package [dplyr]{style="color:indianred;"}
...
::: notes
What did we do? Let's look at the dimension
...
::: notes
so we have our 7 rows and the 13 colums, with missing value
:::
...
A safer option is to use *joins* to combine several tables by columns.]{.fragment}

::: notes
what did we do to make the subset?

show what the cbind does on the blackboard
:::
...
[[A key is a column (or several of columns) that is used to match rows when joining two datasets.]{style="font-size: 30px"}]{.fragment}
...
::: notes 
show what is a key on the backboard, and explain how it is different from a bind_cols

we are going to see the 4 types of join

say here that very important to check the dim of the output
:::
...
here show what is the 1st data set (traits_sub) and the second (sp_com_sub), so we will keep all the rows in traits_sub => see the dim

show tha the coluns have been matches, and that the key is not repeated 
:::
...
::: {.column width="40%"}
The [inner_join]{style="color:indianred;"} keeps only rows from *traits_sub* that have a matching key in *sp_com_sub*.
:::
...
::: {.column width="40%"}
The [full_join]{style="color:indianred;"} keeps all the rows in traits_sub and sp_com_sub
:::
...
But what to do when data are not tidy? 
...
you see that several dataset have been added
::: 
...
These variables are NOT stored in columns.]{.fragment}


## pivot_longer

We can use the function [pivot_longer]{style="color:indianred;"} to put the table in a  tidy format:
...
the columns created have some not very informative names...
:::
...
there are two problems, let's start by the taxo => what would we like to have?
:::
...
why do we have 6 row? let's go back to the previous slide and count them
:::
...
note that this is not needed to make the data tidy
:::
...
Let's use the dataset *dt4*, which is about secondary forests.
...
fct_other => keep some levels and put the other ones into a level called "other" (or another name)

fct_lump => group levels base on a criteria: ex all the levels that have a too small occurences grouped together
:::
...
dt4$past_landuse %>% fct_reorder(
  dt4$age,
  median) %>%  # re-order the levels according the median of the age 
  levels()
```
:::

[[💡 *This is usefull for presenting the categories in a specific order in a graph.*]{style="font-size: 25px"}]{.fragment}
...
## Function [if_else]{style="color:indianred;"} {.smaller}
...
dt4 %>% 
  mutate(age_cat = # add a new column
         ...
         age_cat = as.factor(age_cat)) # transform to factor
```

::: notes
here if_gives a character so we then transform it to a factor

her we perfom one test => so it created to category, one for the results that are TRUE and one for the ones that are FALSE
:::

## Function [case_when]{style="color:indianred;"} {.smaller}

[case_when]{style="color:indianred;"} from the package [dplyr]{style="color:indianred;"} generalises [if_else]{style="color:indianred;"} to allow several conditions:

```{r}
dt4 %>% 
  mutate(category = 
           case_when(dt4$age < 8 ~ # test for the first case
               "Young", # value for the first case
             dt4$age > 10 ~ # test for the second case
               "Old", # value for the second case
             TRUE ~ "Medium")) %>% # all the other cases
  mutate(category = as.factor(category)) # transform to factor
```

::: notes
Her we do several tests sequentially => the the created factors can have more than 2 categories
:::

## Function [case_when]{style="color:indianred;"} {.smaller}

The tests performed by [case_when]{style="color:indianred;"} and [if_else]{style="color:indianred;"} can be combinations of tests (using operators like [&]{style="color:indianred;"}, [|]{style="color:indianred;"}, and [!]{style="color:indianred;"}):
...
## Split a character string {.smaller}
...
[[*NB: if we want to separate a column of a data frame, we can also use [separate]{style="color:indianred;"}.*]{style="font-size: 30px"}]{.fragment}

[[*But [str_split]{style="color:indianred;"} is more flexible as it allows the number of splits to vary between values.*]{style="font-size: 30px"}]{.fragment}

:::
:::::

::: notes
But [str_split] is more flexible has it allows the number of split to vary between values => ex: some values split in 2, some split in 3...

you will see this in the exercice
:::
...
# Dealing with dates with the package [lubridate]{style="color:indianred;"}
...
[[in English](https://r4ds.hadley.nz/datetimes.html) and [in Portuguese](https://pt.r4ds.hadley.nz/datetimes.html){preview-link="false"}]{style="font-size: 25px"}
...
::: notes
and now you know how to do all of this with the tidyverse
:::
...
* Wickham H., Cetinkaya-Rundel M., Grolemund G. *R for Data Science* [in English](https://r4ds.hadley.nz/){preview-link="false"} and [in Portuguese](https://pt.r4ds.hadley.nz/){preview-link="false"}
...
* Wickham, H. . (2014). Tidy Data. Journal of Statistical Software, 59(10), 1–23. [here](https://doi.org/10.18637/jss.v059.i10){preview-link="false"}

* Wickham H., Cetinkaya-Rundel M., Grolemund G. *R for Data Science* [in English](https://r4ds.hadley.nz/) and [in Portuguese](https://pt.r4ds.hadley.nz/){preview-link="false"}
