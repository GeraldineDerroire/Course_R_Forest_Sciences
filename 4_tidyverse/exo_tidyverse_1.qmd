...
* Now, read the data using the function read_csv. 
This is a preferred alternative
as is returns directly a tibble

```{r, message=FALSE}
dt_WG <- read_csv("data/KarnatakaForest.csv")
```
...
# alternative solution (not seen in the class, will be seen later)
dt_WG <- dt_WG %>% 
...
# alternative solution that return a value and not a tibble
n_distinct(big5)
...
n_75 <- dt_WG %>% 
  ...
  distinct(family) %>% # keep only a unique row for each family
