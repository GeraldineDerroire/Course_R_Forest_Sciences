...
```{r}
sp_commer <- read.csv("data/commercial_sp.csv")
class(sp_commer)
```
...
```{r}
as.tibble(rownames_to_column(sp_commer))
```
...
Let's look at the sp_commer data :
...
```{r}
filter(sp_commer, Aggregative == TRUE)
```
